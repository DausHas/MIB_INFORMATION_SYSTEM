...
$find_article = mysqli_query($conn,"SELECT * FROM articles WHERE `a_content` LIKE '%$search%'");
while($row = mysqli_fetch_assoc($find_article))
{
  
  $content = substr($row['a_content'], 0, 900);;
  $id = $row['a_id'];

  echo "<h5 style='text-align:center; font-size:20px;'> Search Result:<br><hr></h5><p style='text-align:center; font-size:20px;'><a href ='mibintroduction_user.php?a_id=$id'>$content</a>...<br><hr></p>";
}
